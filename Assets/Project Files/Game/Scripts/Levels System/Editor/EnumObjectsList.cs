    ...
    public class EnumObjectsList
    {
        ...
        private void DefaultView()
        {
            ...
            for (int index = 0; index < array.arraySize; index++)
            {
                ...
                {
                    CheckPrefab(tempProperty.FindPropertyRelative(objectReferencePropertyPath));
                }
        ...
        private void CheckPrefab(SerializedProperty prefabProperty)
        {
            ...
            if (((GameObject)prefabProperty.objectReferenceValue).GetComponent(customType) == null)
            {
                EditorUtility.DisplayDialog($"{customType.Name} missing error", $"Every prefab needs to have {customType.Name} component. Please add {customType.Name} component and try again.", "OK");
        ...
        private void EditEnumView()
        {
            ...
            if(errors.Count > 0)
            {
                EditorGUILayout.HelpBox($"There are {errors.Count} validation errors. First validation error: {errors[0]}", MessageType.Error);
            }
        ...
        private void ValidataValue(int index, EditedEnumValue editedEnumValue)
        {
            ...
            {
                errors.Add($"\"Name\" element  #{(index + 1)} didn`t pass regex check. To pass regex check you need to start \"name\" with big latin letter and use only latin letters and numbers.");
            }
            else
            {
